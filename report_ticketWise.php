...
<div class="container" style="padding-top:20px;">
	  ...
	  <?php } ?>
	  <h4>Ticket status wise report</h4>
...
if(isset($_POST['btnsubmit']))
{
	...
	$sql =	"   select
				...
				sum(if(`status`='Under Investigation',1,0)) `Under Investigation`,
				sum(if(`status`='Not Reached',1,0)) `Not Reached`,
				sum(if(`status`='Closed',1,0)) `Closed`
				from cust_survey
				where survey_date between '$startdate' and '$enddate'";

	//SELECT employee_name ,msisdn_txt ,con_type ,quest_one_ans ,quest_two_ans,quest_two_others ,VOCdetailsStoryline,txtstory,survey_date,survey_time ,callback FROM cust_survey WHERE survey_date BETWEEN  AND ";
