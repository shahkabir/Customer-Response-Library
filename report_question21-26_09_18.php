...
if(isset($_POST['btnsubmit']))
{
	...
	echo $sql =	"   select distinct(quest_two_ans) ans, count(quest_two_ans) cnt
				from cust_survey
	...
	else
	{
		$updated_arr = array();
		$arr_const = array(
						 	array('con' => 'Internet', 'val' =>100
						 		),
						 	array('con' => 'Value added services', 'val' =>0
						 		),
						 	array('con' => 'Call rate & BL offers', 'val' =>0
						 		),	
						 	array('con' => 'Others', 'val' =>0
						 		),
						 	array('con' => 'Network', 'val' =>0
						 		)
						 );

			echo 'VAL:'.$arr_const['con']['val'];			
			...
			while ($row = mysql_fetch_array($sq)) 
			{
				...
				{
					if(in_array_r($v,$arr_const))
					{
						//echo '<br/>found: '.$v;
						//echo '<br/>$arr_const[$v][val]'.$arr_const[$v]['val'];
						//echo '<br/>$row[cnt]'.$row['cnt'];

						$arr_const[$v]['val'] = $arr_const[$v]['val'] + $row['cnt'];

						//$updated_arr[$v]['val'] = $arr_const[$v]['val'] + $row['cnt'];
